...
export default function CountdownTimer({ ttlSeconds, expiresAt, onExpire, size = 'md' }) {
  ...
  const color  = isExpired ? 'text-rose-400'   : isCritical ? 'text-rose-400'   : isWarning ? 'text-amber-400'   : 'text-emerald-400';
  const bg     = isExpired ? 'bg-rose-500/10'   : isCritical ? 'bg-rose-500/10'   : isWarning ? 'bg-amber-500/10'   : 'bg-emerald-500/10';
  const border = isExpired ? 'border-rose-500/20' : isCritical ? 'border-rose-500/20' : isWarning ? 'border-amber-500/20' : 'border-emerald-500/20';
  ...
  return (
    <div className={`${bg} border ${border} rounded-2xl ${s.pad} ${pulse}`}>
